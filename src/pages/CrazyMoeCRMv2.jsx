...
import {
  ...
  ListChecks, AlertTriangle, Calendar, Target
} from "lucide-react";
...
function genSample() {
  ...
  return raw.map((r, i) => {
    const msgs = r.m.map(([sender, content, ts]) => ({ id: `s${i}_${ts}`, sender, content, ts, hasPhoto: false, hasShare: false, reactions: [] }));
    return { id: `s_${i}`, title: r.t, participants: [me, r.t], messages: msgs, lastTs: msgs.at(-1).ts, firstTs: msgs[0].ts, msgCount: msgs.length, product: r.p };
  });
}
...
function getSampleMeta() {
  ...
  return {
    ...
    s_10: { status:'waiting on customer', tags:['follow-up'],           notes:'Needs a truck. Check back this week', nad: d2 },
...
const KEYS = { CONV: 'cm_conv2', CUST: 'cm_cust2', FILTERS: 'cm_filters2', MYNAME: 'cm_myname' };
...
function UploadView({ onLoad, loading, hasData, onContinue }) {
  ...
  const processFiles = async files => {
    ...
    const all = [];
    for (const f of arr) {
      try { const t = await f.text(); all.push(...parseFBExport(JSON.parse(t))); } catch {}
    }
    if (all.length) onLoad(all, detectMyName(all), {});
  };
  return (
    <div style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:32, gap:24, overflow:'auto' }}>
      ...
      <div onDragOver={e=>{e.preventDefault();setDrag(true);}} onDragLeave={()=>setDrag(false)}
        ...
        <input ref={ref} type="file" multiple accept=".json" style={{ display:'none' }} onChange={e=>processFiles(e.target.files)} />
      ...
      <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:'20px 24px', maxWidth:500, width:'100%' }}>
        ...
        {['Facebook Settings → Your Facebook Information','Download Your Information → Messages','Format: JSON · Date range: All Time','Download and extract the ZIP','Upload .json files from the inbox/ folder'].map((s,i)=>(
...
// INBOX VIEW
// ═══════════════════════════════════════════
function InboxView({ convs, myName, convMeta, setConvMeta, onOpenCustomer, globalSearch }) {
  ...
  useEffect(()=>{ if(convs.length&&!selected) setSelected(convs[0]); },[convs]);
  useEffect(()=>{ msgEndRef.current?.scrollIntoView({behavior:'smooth'}); },[selected?.id]);
  useEffect(()=>{
    if(selected){ setNoteText(convMeta[selected.id]?.notes||''); setEditNote(false); setAddingTag(false); setShowNad(false); }
  },[selected?.id]);
  ...
  const sel = selected ? convs.find(c=>c.id===selected.id)||selected : null;
        ...
        <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden' }}>
          ...
          <div style={{ flex:1, overflow:'auto', padding:'16px 20px', display:'flex', flexDirection:'column', gap:4 }}>
            {sel.messages.map((msg,i)=>{
              ...
              const prevSender=i>0?sel.messages[i-1].sender:null;
              const showAvatar=!isMe&&prevSender!==msg.sender;
              const showDate=i===0||new Date(msg.ts).toDateString()!==new Date(sel.messages[i-1].ts).toDateString();
              return (
                <div key={msg.id}>
...
// CONVERSATIONS VIEW
// ═══════════════════════════════════════════
function ConversationsView({ convs, convMeta, setConvMeta, onOpenCustomer, globalSearch, savedFilters, setSavedFilters }) {
  const [search, setSearch] = useState('');
  const [filterStatus, setFilterStatus] = useState('');
  ...
  const filtered=useMemo(()=>{
    ...
    return list;
  },[convs,search,globalSearch,filterStatus,filterTag,filterProduct,filterOverdue,sortBy]);

  const applyFilter=f=>{ setFilterStatus(f.filters.status||''); setFilterTag((f.filters.tags||[])[0]||''); setFilterOverdue(!!f.filters.overdue); };
  const saveFilter=()=>{
    if(!saveName.trim()) return;
    const f={id:`f_${Date.now()}`,name:saveName,filters:{status:filterStatus,tags:filterTag?[filterTag]:[],overdue:filterOverdue}};
  ...
  return (
    <div style={{ flex:1, overflow:'hidden', display:'flex', flexDirection:'column' }}>
      <div style={{ padding:'10px 20px', borderBottom:`1px solid ${C.border}`, display:'flex', gap:8, alignItems:'center', flexWrap:'wrap', flexShrink:0, background:C.card }}>
        ...
        {(filterStatus||filterTag||filterProduct||filterOverdue)&&<Btn variant="ghost" onClick={()=>{setFilterStatus('');setFilterTag('');setFilterProduct('');setFilterOverdue(false);}} style={{ fontSize:11, color:C.red }}><X size={12}/>Clear</Btn>}
      ...
      <div style={{ padding:'7px 20px', borderBottom:`1px solid ${C.border}`, display:'flex', gap:6, flexWrap:'wrap', alignItems:'center', flexShrink:0 }}>
        {savedFilters.map(f=>(
          <button key={f.id} onClick={()=>applyFilter(f)} style={{ background:C.card2, border:`1px solid ${C.border}`, borderRadius:100, padding:'3px 12px', fontSize:11, color:C.mutedLight, cursor:'pointer', fontWeight:600, display:'flex', alignItems:'center', gap:5 }}>
            {f.name}<span onClick={e=>{ e.stopPropagation(); const n=savedFilters.filter(x=>x.id!==f.id); setSavedFilters(n); }} style={{ opacity:0.5, fontSize:13, lineHeight:1 }}>×</span>
          </button>
        ))}
      ...
      <div style={{ flex:1, overflow:'auto' }}>
        ...
        {!filtered.length&&<div style={{ textAlign:'center', padding:48, color:C.muted }}>No conversations match</div>}
      </div>
    </div>
  );
}
    ...
    <>
      ...
      <div style={{ position:'absolute', top:0, right:0, bottom:0, width:380, background:C.card, borderLeft:`1px solid ${C.border}`, zIndex:50, display:'flex', flexDirection:'column', boxShadow:'-8px 0 40px #00000060' }}>
        ...
        <div style={{ flex:1, overflow:'auto', padding:20, display:'flex', flexDirection:'column', gap:16 }}>
          {/* Next Action Date (customer level) */}
          <div>
...
function PipelineView({ convs, convMeta, setConvMeta, onOpenCustomer }) {
  ...
  const COLS = [
    ...
    { id:'waiting on customer',  label:'Waiting',        short:'Waiting' },
    { id:'waiting on me',        label:'My Turn',        short:'My Turn' },
    { id:'follow-up',            label:'Follow-up',      short:'Follow-up' },
    { id:'sold',                 label:'Sold ✅',        short:'Sold' },
      ...
      {COLS.map(col=>{
        ...
        return (
          <div key={col.id}
            ...
            onDrop={e=>{
              e.preventDefault();
              if(dragId.current && dragId.current!==col.id) upd(dragId.current,{status:col.id});
            ...
            <div style={{ padding:8, display:'flex', flexDirection:'column', gap:6, flex:1, overflow:'auto', minHeight:120 }}>
                    ...
                    {editingNad===conv.id&&(
                      <div onClick={e=>e.stopPropagation()} style={{ marginTop:4 }}>
                        <input type="date" value={conv.nad||''} onChange={e=>{ upd(conv.id,{nad:e.target.value}); setEditingNad(null); }}
                          style={{ background:C.card2, border:`1px solid ${C.borderLight}`, borderRadius:6, padding:'3px 8px', color:C.text, fontSize:11, outline:'none', width:'100%', boxSizing:'border-box', colorScheme:'dark' }}
                          autoFocus/>
                      </div>
                    )}
              ...
              {colConvs.length===0&&(
                <div style={{ textAlign:'center', color:C.muted, fontSize:11, padding:'24px 0', opacity:0.4, borderRadius:6, border:`1px dashed ${C.border}` }}>
                  Drop here
                </div>
              )}
            </div>
...
function QueueView({ convs, convMeta, setConvMeta, onOpenCustomer }) {
  ...
  const queue     = convs
    .filter(c=>['follow-up','waiting on me','active'].includes(c.status)&&!used.has(c.id))
    .sort((a,b)=>{
      if(a.nad&&b.nad) return a.nad.localeCompare(b.nad);
      if(a.nad) return -1; if(b.nad) return 1;
...
export default function App() {
  ...
  const [convMeta, setConvMetaRaw] = useState(()=>lsG(KEYS.CONV)||{});
  ...
  const setConvMeta = useCallback(u=>setConvMetaRaw(p=>{ const n=typeof u==='function'?u(p):u; lsS(KEYS.CONV,n); return n; }),[]);
  ...
  return (
    <div style={{ background:C.bg, minHeight:'100vh', color:C.text, fontFamily:'"SF Pro Display","Segoe UI",system-ui,-apple-system,sans-serif', display:'flex', flexDirection:'column', fontSize:13 }}>
      ...
      <div style={{ flex:1, overflow:'hidden', display:'flex', position:'relative' }}>
        ...
        {view==='dashboard'    && <DashboardView convs={finalConvs} customers={customers} onViewChange={(v)=>setView(v)}/>}
        {view==='inbox'        && <InboxView convs={finalConvs} myName={myName} convMeta={convMeta} setConvMeta={setConvMeta} onOpenCustomer={setCustomerPanel} globalSearch={globalSearch}/>}
        ...
        {view==='conversations'&& <ConversationsView convs={finalConvs} convMeta={convMeta} setConvMeta={setConvMeta} onOpenCustomer={setCustomerPanel} globalSearch={globalSearch} savedFilters={savedFilters} setSavedFilters={setSavedFilters}/>}
        ...
        {customerPanel&&(
          <CustomerPanel
            ...
            onOpenConv={()=>{ setView('inbox'); setCustomerPanel(null); }}
          />
        )}
      </div>
    </div>
