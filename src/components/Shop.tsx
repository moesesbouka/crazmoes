...
// Re-use the ShopifyProduct type so all existing child components work unchanged
...
const Shop = () => {
  ...
  useEffect(() => {
    ...
    fetchActiveListingsProgressive((listings, isComplete) => {
      ...
      // Convert to ShopifyProduct shape so ProductCard/ProductGrid need no changes
  ...
  const filteredAndSorted = useMemo(() => {
    ...
    result.sort((a, b) => {
      switch (sortOption) {
        ...
        case "newest":
        default:
          return 0; // already sorted newest-first from Supabase
      }
    });
  ...
  const productsByCategory = useMemo(() => {
    ...
    filteredAndSorted.forEach((product) => {
      const cat = resolveProductCategory(
        product.node.title,
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="py-12 md:py-16 relative overflow-hidden">
        ...
        <div className="container text-center">
          ...
          <p
            className="text-muted-foreground text-lg max-w-2xl mx-auto animate-fade-in-up opacity-0"
            style={{ animationDelay: "100ms" }}
          >
            {isLoading
              ? "Loading inventory…"
              : `${filteredAndSorted.length.toLocaleString()} items available for Buffalo pickup`}
          </p>
        </div>
      </section>
          ...
          <div className="space-y-8">
            {productsByCategory.map(({ category, products: catProducts }, idx) => {
              const startIndex = productsByCategory
                .slice(0, idx)
                .reduce((acc, c) => acc + c.products.length, 0);
              return (
                <CategorySection
                  key={category}
                  category={category}
