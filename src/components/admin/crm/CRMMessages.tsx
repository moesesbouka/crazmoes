...
import { Search, Download, X, Save, Bookmark } from "lucide-react";
import { useCRMStore, filterMessages, exportToCSV, type CRMMessage } from "@/lib/crmStore";
import { useCRMMetadataStore, type SavedFilter, DEFAULT_SAVED_FILTERS } from "@/lib/crmMetadataStore";
import { toast } from "sonner";

export function CRMMessages() {
  const { messages, filters, setFilters, resetFilters } = useCRMStore();
  const { savedFilters, addSavedFilter, removeSavedFilter } = useCRMMetadataStore();
  ...
  const loadPreset = (preset: SavedFilter) => {
    const f = preset.filter as Record<string, any>;
    // Map saved filter keys to CRM filters
    if (f.tags?.length) setFilters({ search: '', product: '', customer_name: '' });
    if (f.statuses?.length) setFilters({ search: '' });
    if (f.hasNotes) setFilters({ search: '' });
    if (f.hasTags) setFilters({ search: '' });
    ...
    <div className="space-y-4">
      <div className="flex gap-2">
        <div className="relative flex-1">
          ...
          <Input placeholder="Search messages..." className="pl-9" value={filters.search} onChange={(e) => { setFilters({ search: e.target.value }); setPage(0); }} />
        </div>
      ...
      <div className="flex flex-wrap gap-2">
        {chipFilters.map((c) => (
          <Badge key={c.key} variant={c.active ? "default" : "outline"} className="cursor-pointer select-none" onClick={() => { setFilters({ [c.key]: !c.active }); setPage(0); }}>{c.label}</Badge>
        ))}
        <Button variant="ghost" size="sm" className="h-6 text-xs" onClick={() => { resetFilters(); setPage(0); }}><X className="h-3 w-3 mr-1" /> Reset</Button>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {savedFilters.map((p) => (
          <div key={p.id} className="flex items-center gap-0.5">
            <Badge variant="secondary" className="cursor-pointer text-xs" onClick={() => loadPreset(p)}>{p.name}</Badge>
            {!p.id.startsWith('preset-') && (
              <button className="text-muted-foreground hover:text-destructive" onClick={() => removeSavedFilter(p.id)}><X className="h-3 w-3" /></button>
            )}
          </div>
        ))}
        <div className="flex items-center gap-1">
          <Input className="h-6 w-24 text-xs" placeholder="Save as…" value={presetName} onChange={(e) => setPresetName(e.target.value)} />
          <Button variant="ghost" size="sm" className="h-6 px-2" onClick={() => { if (presetName.trim()) { addSavedFilter(presetName.trim(), { ...filters }); setPresetName(''); toast.success('Filter saved'); } }}>
        ...
        <CardContent className="p-0">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {paged.map((m, i) => (
                  <tr key={`${m.thread_path}-${m.timestamp_ms}-${i}`} className="border-b border-border/30 hover:bg-muted/30 cursor-pointer transition-colors" onClick={() => setSelectedMsg(m)}>
                    ...
                    <td className="py-2 px-3 text-xs truncate max-w-[120px]"><span className={m.owner_message === 1 ? 'text-primary font-medium' : ''}>{m.sender}</span></td>
      ...
      <Sheet open={!!selectedMsg} onOpenChange={() => setSelectedMsg(null)}>
        <SheetContent className="w-[500px] sm:w-[600px] overflow-y-auto">
          {selectedMsg && (
            <>
              <SheetHeader><SheetTitle className="text-base">{selectedMsg.listing_title || 'Message Detail'}</SheetTitle></SheetHeader>
              <div className="mt-4 space-y-4">
                <div className="grid grid-cols-2 gap-3 text-sm">
                  {[
                    ['Sender', selectedMsg.sender], ['Customer', selectedMsg.customer_name || '-'],
                    ['Listing', selectedMsg.listing_title], ['Thread', selectedMsg.thread_path],
                    ['Timestamp', selectedMsg.timestamp_iso], ['Attachments', String(selectedMsg.attachments_count)],
                  ].map(([label, val]) => (
                    <div key={label}><p className="text-xs text-muted-foreground">{label}</p><p className="text-sm truncate">{val}</p></div>
                  ))}
                </div>
                  ...
                  <div className="space-y-2 max-h-[400px] overflow-y-auto">
                    {threadMessages.map((m, i) => (
                      <div key={i} className={`rounded-lg p-2.5 text-xs ${
                        m.timestamp_ms === selectedMsg.timestamp_ms ? 'bg-primary/10 border border-primary/30' :
