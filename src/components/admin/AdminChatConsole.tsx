...
import {
  ...
  Clock, Paperclip, StickyNote, ListFilter, Flame, Ban, Truck, Star
} from "lucide-react";
...
type InboxFilter =
  | "all" | "recent" | "customer-only" | "attachments" | "has-notes"
  | "follow-up-queue" | "hot-leads" | "pending-pickup" | "vip"
  | "no-response-3d" | "overdue";

const INBOX_FILTERS: { v: InboxFilter; label: string; icon?: any }[] = [
  { v: "all", label: "All" },
  { v: "follow-up-queue", label: "Follow-up Q", icon: ListFilter },
  { v: "overdue", label: "Overdue", icon: Clock },
  { v: "customer-only", label: "Needs Reply", icon: User },
  { v: "no-response-3d", label: "No Reply 3d", icon: Ban },
  { v: "hot-leads", label: "Hot Leads", icon: Flame },
  { v: "pending-pickup", label: "Pickup", icon: Truck },
  { v: "vip", label: "VIP", icon: Star },
...
export function AdminChatConsole() {
  ...
  const conversations = useMemo(() => {
    ...
    for (const m of messages) {
      ...
      if (m.owner_message !== 1 && m.system_message !== 1) {
        entry.summary.customerMessageCount++;
        if (m.timestamp_ms > entry.summary.lastCustomerTimestamp) entry.summary.lastCustomerTimestamp = m.timestamp_ms;
      }
  ...
  const filteredConversations = useMemo(() => {
    ...
    switch (inboxFilter) {
      ...
      case "hot-leads": list = list.filter((c) => getConversationMeta(c.summary.thread_path).tags.includes('hot-lead')); break;
      case "pending-pickup": list = list.filter((c) => getConversationMeta(c.summary.thread_path).tags.includes('pending-pickup')); break;
      case "vip": {
        const { customerMeta } = useCRMMetadataStore.getState();
        const vipNames = new Set(Object.entries(customerMeta).filter(([_, m]) => m.tags?.includes('vip')).map(([n]) => n));
        list = list.filter((c) => vipNames.has(c.summary.customer_name));
        break;
      }
      case "no-response-3d":
        list = list.filter((c) => {
          const meta = getConversationMeta(c.summary.thread_path);
          if (CLOSED_STATUSES.has(meta.status)) return false;
          return c.summary.lastCustomerTimestamp > 0 && !c.summary.isOwnerLast && c.summary.lastCustomerTimestamp < threeDaysAgo;
        });
        break;
      case "overdue":
        list = list.filter((c) => {
          const meta = getConversationMeta(c.summary.thread_path);
          if (CLOSED_STATUSES.has(meta.status) || !meta.nextActionDate) return false;
          const d = parseISO(meta.nextActionDate);
          return isValid(d) && isBefore(d, today);
        });
        list = [...list].sort((a, b) => {
          const aD = getConversationMeta(a.summary.thread_path).nextActionDate;
          const bD = getConversationMeta(b.summary.thread_path).nextActionDate;
      ...
      case "follow-up-queue":
        list = list.filter((c) => {
          const meta = getConversationMeta(c.summary.thread_path);
          return meta.status === 'follow-up' || meta.status === 'waiting-on-me' || meta.status === 'active';
        });
        list = [...list].sort((a, b) => {
    ...
    return (
      <Card className="border-dashed border-2 border-border/50 bg-card/30">
        <CardContent className="p-16 flex flex-col items-center gap-4 text-center">
          ...
          <p className="text-sm text-muted-foreground">Upload a Messenger CSV or JSON in the <strong>CRM</strong> tab first.</p>
        </CardContent>
      </Card>
    );
  }
