...
import {
  ...
  Clock, Paperclip, Filter, ChevronRight
} from "lucide-react";
...
type InboxFilter = "all" | "recent" | "customer-only" | "attachments";
...
export function AdminChatConsole() {
  ...
  const filteredConversations = useMemo(() => {
    ...
    switch (inboxFilter) {
      case "recent":
        list = list.filter((c) => now - c.summary.lastTimestamp < threeDays);
        break;
      case "customer-only":
        list = list.filter((c) => !c.summary.isOwnerLast);
        break;
      case "attachments":
        list = list.filter((c) => c.summary.hasAttachments);
        break;
    }
  ...
  return (
    <div className="grid md:grid-cols-[340px_1fr] gap-0 h-[calc(100vh-14rem)] border border-border rounded-xl overflow-hidden bg-card">
      ...
      <div className="flex flex-col border-r border-border bg-secondary/30">
        {/* Header */}
        <div className="p-3 border-b border-border space-y-2.5 flex-shrink-0">
          ...
          <div className="relative">
            ...
            <Input
              placeholder="Search name, listing, message…"
              value={search}
          ...
          <ToggleGroup
            type="single"
            value={inboxFilter}
            onValueChange={(v) => v && setInboxFilter(v as InboxFilter)}
            className="justify-start gap-1"
          >
            <ToggleGroupItem value="all" className="h-6 px-2 text-[10px] rounded-full data-[state=on]:bg-primary data-[state=on]:text-primary-foreground">
              All
            </ToggleGroupItem>
            <ToggleGroupItem value="recent" className="h-6 px-2 text-[10px] rounded-full data-[state=on]:bg-primary data-[state=on]:text-primary-foreground">
              <Clock className="h-3 w-3 mr-1" />Recent
            </ToggleGroupItem>
            <ToggleGroupItem value="customer-only" className="h-6 px-2 text-[10px] rounded-full data-[state=on]:bg-primary data-[state=on]:text-primary-foreground">
              <User className="h-3 w-3 mr-1" />Needs Reply
            </ToggleGroupItem>
            <ToggleGroupItem value="attachments" className="h-6 px-2 text-[10px] rounded-full data-[state=on]:bg-primary data-[state=on]:text-primary-foreground">
              <Paperclip className="h-3 w-3 mr-1" />Files
            </ToggleGroupItem>
          </ToggleGroup>
        </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-1 mb-1">
                    ...
                    <span className="text-[11px] text-muted-foreground truncate">{s.listing_title || "—"}</span>
                  </div>
                  <div className="flex items-center justify-between gap-2">
                    <p className="text-[11px] text-muted-foreground/70 truncate flex-1">
                      {s.isOwnerLast && <span className="text-muted-foreground">You: </span>}
                      {s.lastMessage}
                    </p>
                    <div className="flex items-center gap-1.5 flex-shrink-0">
                      {s.hasAttachments && <Paperclip className="h-3 w-3 text-muted-foreground/50" />}
                      <Badge variant="outline" className="h-4 min-w-[1.25rem] flex items-center justify-center px-1 text-[9px] font-mono">
                        {s.messageCount}
                      </Badge>
                    </div>
                  </div>
        ...
        <div className="px-4 py-3 border-b border-border flex items-center justify-between flex-shrink-0">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[11px] text-muted-foreground truncate flex items-center gap-1">
                    <Package className="h-3 w-3 inline" /> {selectedSummary.listing_title}
          ...
          {selectedThread && (
            <div className="flex items-center gap-2 flex-shrink-0">
              <Switch id="show-system" checked={showSystem} onCheckedChange={setShowSystem} className="scale-75" />
          ...
          <ScrollArea className="flex-1 px-4 py-3">
            <div className="space-y-2">
              {threadMessages.map((msg, i) => {
                ...
                // Date separator
                const showDateSep =
                  i === 0 || threadMessages[i - 1].message_date !== msg.message_date;
