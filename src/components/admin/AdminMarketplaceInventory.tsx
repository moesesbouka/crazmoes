...
export function AdminMarketplaceInventory() {
  ...
  const fetchListings = useCallback(async () => {
    ...
    try {
      ...
      // Hide broken imports by default (missing/Untitled title or missing images)
      if (!showBroken) {
        query = query.not("title", "is", null).neq("title", "");
        query = query.not("title", "ilike", "untitled");
  ...
  }, [currentPage, searchQuery, sortField, sortDirection, accountFilter, statusFilter, conditionFilter, launchFilter, showBroken]);
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex flex-wrap gap-4">
          ...
          <Button
            variant={showBroken ? "default" : "outline"}
            size="sm"
            onClick={() => { setShowBroken(!showBroken); setCurrentPage(1); }}
            title="Toggle showing broken imports (Untitled / missing images)"
          >
            <AlertTriangle className="h-4 w-4 mr-2" />
            {showBroken ? "Showing Broken" : "Hide Broken"}
          </Button>
